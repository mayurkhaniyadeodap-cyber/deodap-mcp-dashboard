  ...
  return (
    <div className="space-y-6">
      ...
      <ChartCard
        ...
        loading={false}
        height={280}
        action={<SourceBadge status={rec?.source === "live" ? "live" : "sample"} />}
      >
        {recovery.isLoading ? (
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
        {/* Daily orders + value */}
        <ChartCard title="Daily Order Value" description={`Order value per day · by ${data?.date_field ?? "order_date"}`} loading={isLoading} height={300} className="lg:col-span-2" action={<SourceBadge status={badge} />}>
          <ResponsiveContainer width="100%" height="100%">
        ...
        {/* Daily order volume */}
        <ChartCard title="Daily Orders" description={`Order count per day · ${formatNumber(daily.reduce((s, d) => s + d.orders, 0))} total`} loading={isLoading} height={300} action={<SourceBadge status={badge} />}>
          <ResponsiveContainer width="100%" height="100%">
